...
function SaveRights() {
    ...
    if (isSuccess) {
        ...
        var postCall = $.post(GetSiteURL() + "/PackageRights/SavePackageRights", $('#fPackageRightDetail').serialize());
        postCall.done(function (data) {
            HideLoader();
            if (data.Status == true) {
                ShowMessage(data.Message, 'Success');
                setTimeout(function () { window.location.href = "PackageMaintenance?deptID=0"; }, 2000);
...
function Remove(deptID) {
    if (confirm('Are you sure want to delete this?')) {
        ShowLoader();
        var postCall = $.post(GetSiteURL() + "/PackageRights/Remove", { "deptID": deptID });
        postCall.done(function (data) {
            HideLoader();
            if (data.Status == true) {
                ShowMessage(data.Message, 'Success');
